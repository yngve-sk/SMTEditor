...
public class SharedMulticastTree {
     ...
     * @param links
     *            the list of the nodes neighbor lists, must be of same length
     *            as nodes and correspond to the nodes list.
     * @param numberOfDestinations
     *            number of destinations
     */
    public SharedMulticastTree(List<Point2D> nodes, List<List<Integer>> links, int numberOfDestinations)
	    throws IllegalArgumentException {
	if (nodes.size() != links.size())
	    throw new IllegalArgumentException(
		    "Amount of nodes and amount of neighbor lists must be of same length, nodes.size() = "
			    + nodes.size() + ", links.size() = " + links.size());

	if (numberOfDestinations > nodes.size())
	    throw new IllegalArgumentException(
		    "Number of destination = " + numberOfDestinations + ", number of nodes = " + nodes.size());
    ...
    /**
     * 
     * @return minimum X and Y value for all nodes, used for calculating layout
     *         in GUI
     */
    ...
    /*
     * public double getMinX() { double min = Double.MAX_VALUE; for(SMTNode n :
     * this.nodes.values()) if(n.getX() <= min) min = n.getX();
     * 
     * return min; }
     * 
     * public double getMinY() { double min = Double.MAX_VALUE; for(SMTNode n :
     * this.nodes.values()) if(n.getX() <= min) min = n.getX();
     * 
     * return min; }
     */
    ...
    /**
     * Iterates over all the neighbor lists and copies the links into the set of
     * distinct links
     */
    ...
    /**
     * Adds a node to the tree. This will result in an increment of the node id
     * tracker
     * 
     * @param x
     * @param y
     * @param isDestination
     * @param nextNodeId
     *            the upcoming id must be passed in here, i.e
     *            IdTracker.getNextNodeId()
     * @param neighbors
	...
	double roundedX = x >= 0 ? x : 0;
	double roundedY = y >= 0 ? y : 0;
	nodes.put(nextNodeId, SMTNodeFactory.newNode(roundedX, roundedY, isDestination));
	if (neighbors != null) {
    ...
    /**
     * Removes a node from the tree, from all its neighbors neighbor lists, all
     * the links associated with the node.
     * 
     * @param id
     *            the node
     */
	...
	nodes.get(id).relocate(x >= 0 ? x : 0, y >= 0 ? y : 0);
	recalculate();
    }
    ...
    /**
     * @return All the nodes, node cost and links lie within the sMTNodes
     *         (assuming a calculation has been done)
     */
    ...
    /**
     * Recalculates the value and returns the time it took in milliseconds. The
     * clearing of node data is not counted into the calculation.
     * 
     * @return the time of the recalculation
     */
    ...
    /**
     * Updates power levels and most distant links, ensuring they are stored in
     ...
     *
     * @param id
     * @return gets the two power levels of the node. Highest at index 0, second
     *         highest at index 1
     */
     ...
     * @param n1
     * @param n2
     * @return The power cost of the transmission, taking kappa and gamma into
     *         consideration.
     */
     ...
     *
     * @param id
     * @return the two most distant nodes, the most distant at index 0, second
     *         most distant at index 1
     */
    ...
    /**
     * Tiny class to pass double values through. After passing n doubles through
     * the filter, it will retain the two largest values. All values passed
     * through must be >= 0.
     * 
     * @author Yngve Sekse Kristiansen
     *
	...
	/**
	 * Passes a double value through, if it is greater than the highest, or
	 * second highest (but not equal to) it'll be retained and stored.
	 * 
	 * @param value
	 */
	...
	/**
	 *
	 * @return The highest two values retained in the filter. The highest
	 *         value at index 0, second highest at index 1
	 */
	...
	switch (f) {
	case AVG_NODE_COST:
	    return Double.toString(utils.Math.trim(cost <= 0 ? 0 : cost / nodes.size()));
	case AVG_LINK_LENGTH:
	    return getAverageLinkLength();
	case MOST_EXPENSIVE_NODE:
	    return getMostExpensiveNode();
	case LONGEST_LINK:
	    return getLongestLink();
	case TOTAL_TREE_COST:
	    return this.cost == -1 ? "Invalid SMT" : Double.toString(this.cost);
	case NUM_NODES:
	    return Integer.toString(nodes.size());
	case NUM_LINKS:
	    return Integer.toString(distinctLinks.values().size());
	case NUM_DESTINATIONS:
	    return getNumDestinations();
	case NUM_NONDESTINATIONS:
	    return getNumNonDestinations();
	case CALCULATION_TIME:
	    return Double.toString(calculationTime);
	default:
	    return null;
	}
    }
	...
	return Double.toString(utils.Math.trim(totalLength / distinctLinks.values().size()));
    }
	...
	/**
	 * Gets the two most distant nodes, furthest at index 0, next furthest
	 * at index 1.
	 * 
	 * @return
	 */
	...
	/**
	 * Runs a node through the filter, retains it if its distance is greater
	 * than the next biggest distance currently being retained
	 * 
	 * @param node
	 *            the node
	 */
	public void runThroughFilter(SMTNode node) {
		...
		furthestDistance = dist;
		    ...
		    || (dist == furthestDistance && nextFurthest == null)) {
		nextFurthest = node;
	...
	// subtree size and opposite subtree size should now be stored in each
	// link

	for (SMTNode n : nodes.values())
	    if (n.getMostDistant() != -1) { // same as if j has neighbors or
					    // not?
		SMTLinkKey linkKey = new SMTLinkKey(n.id, n.getMostDistant());
		if (!(!n.isDestination && n.isLeaf())) { // if n isn't a
							 // non-destination leaf
		    double nCost = n.getCost(nod, distinctLinks.get(linkKey)); // pass
									       // link
									       // with
									       // subtree
									       // info
    ...
    /**
     * Sets the link direction to the same direction as the link key, used for
     * avoiding errors with link direction when storing subtree sizes in links.
     * 
     * @param key
     *            the link key
     */
	...
	if (n2.getNeighboursWithinRange().size() == 1) {
	    ...
	    // System.out.println("(nod = " + nod + ")" + "With link direction:
	    // " + subtreeSize + ", opposite: " + (nod - subtreeSize));
	    return subtreeSize;
	}
	...
	if (numLinks >= numNodes - singletons) { // System.out.println("numLinks
						 // >= numNodes - singletons");
	    isValidSMT = false;
	    return;
	}
	...
	if (discreteCellSize != Mode.getDiscreteCellSize()) {
	    ...
	    // only adjust when it already is in discrete mode and only the cell
    ...
    /**
     * When the discrete cell size is changed the nodes will move accordingly
     * (i.e the graph "grows)
     * 
     * @param oldCellSize
     * @param newCellSize
